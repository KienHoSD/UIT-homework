...
void BFS(const vector<priority_queue<int,vector<int>,greater<int>>>& matran, map<string,int>& hash_word_to_index, int sodinh,int socanh){
    ...
    // start to finding path by BFS m's times
    ...
    for(int i=0;i<m_lan_tim_duong;i++){
        ...
        cin>>word1>>word2;
        // ACTIVATE SPACE MODE  
        dang_xet_queue.push(hash_word_to_index[word1]);
        while(!dang_xet_queue.empty()){
            dang_xet_index=dang_xet_queue.front();
            ...
            while(!matrantemp[dang_xet_index].empty()){
                if(dinh_da_xet[matrantemp[dang_xet_index].top()]==false&&previous_of_key_map.count(matrantemp[dang_xet_index].top())==0){
                    ...
                    if(dang_xet_queue.back()==hash_word_to_index[word2]){
                        found_word2=true;
                        break;                    
                    } 
                }
...
int main(){
    ...
    BFS(matran,hash_word_to_index,sodinh,socanh);
}